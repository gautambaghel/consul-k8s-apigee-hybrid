...
set_config_params() {
    ...
    export AX_REGION=${AX_REGION:-'us-west1'}
    echo "- Analytics Region $AX_REGION"

    export REGION=${REGION:-'us-west1'}
    gcloud config set compute/region "$REGION"

    export ZONE=${ZONE:-'us-west1-a,us-west1-b,us-west1-c'}
    ...
    export PRIVATE_CLUSTER=${PRIVATE_CLUSTER:-'false'}
    ...
    export ENV_NAME=${ENV_NAME:="env"}
    echo "- Environment Name $ENV_NAME"
    export ENV_GROUP_NAME=${ENV_GROUP_NAME:="envgroup"}
...
install_certmanager() {
  ...
  echo "Sleep for 30 seconds to ease API server pressure."
  sleep 30
}
